    ...
    if(this.firstname == "") {
      ...
      if(this.pib.length != 9) {
        ...
        if(addressTemp.length != 5) {
          ...
          this.userService.register(this.firstname, this.lastname, this.username, this.password, this.phone, this.email, this.company, state, city, postalCode, street, streetNumber, this.pib, this.companyNumber, this.image).subscribe((json) => {
            let confirmationMessage = json['message'];
            if(confirmationMessage == "OK") {
              this.successMessage = "Registration request successfully sent!";
            }
            else {
